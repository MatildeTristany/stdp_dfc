...
class PredictiveCodingNetwork(object):
    ...
    def train_epoch(self, x_batches, y_batches, epoch_num=None):
        ...
        for batch_id, (x_batch, y_batch) in enumerate(zip(x_batches, y_batches)):
            ...
            if self.DH:
                x, errors, DH_update, _ = self.infer_differentialHebbian(x, batch_size)
                self.update_params_differentialHebbian(
                    x, errors, DH_update, batch_size, epoch_num=epoch_num, n_batches=n_batches, curr_batch=batch_id
                )
            else:
    ...
    def infer_differentialHebbian(self, x, batch_size):
        ...
        DH_update_tensor_shape = [[self.neurons[l+1],self.neurons[l]] for l in range(self.n_layers-1)]
        DH_update = [torch.zeros(DH_update_tensor_shape[l][0],DH_update_tensor_shape[l][1]) for l in range(self.n_layers-1)]
        dx = [[] for _ in range(self.n_layers)]
        ...
        for itr in range(self.itr_max):
            ...
            for l in range(1, self.n_layers - 1):
                ...
                g = torch.mul(self.W[l].T @ errors[l + 1], f_x_deriv_arr[l])
                x[l]  = x[l] + beta * (-errors[l] + g)
                dx[l] = beta * (-errors[l] + g)
                DH_loc = dx[l] @ F.f(x[l-1], self.act_fn).T
                
                DH_update[l-1] +=DH_loc
            # update errors
            f = 0
            for l in range(1, self.n_layers):
                f_x = F.f(x[l - 1], self.act_fn)
        ...
        for l in range(self.n_layers-1):
            DH_update[l] = DH_update[l]/its*2/beta #Scaling is calculated to be consistent with the scaling of the errors.

        return x, errors, DH_update, its
    
    def update_params_differentialHebbian(self, x, errors, DH_update, batch_size, epoch_num=None, n_batches=None, curr_batch=None):
        ...
        for l in range(self.n_layers - 1):
            ...
            grad_w[l] = (
                self.vars[-1] * (1 / batch_size) * DH_update[l]
    ...
    def update_params(self, x, errors, batch_size, epoch_num=None, n_batches=None, curr_batch=None):
        ...
        for l in range(self.n_layers - 1):
            ...
            grad_b[l] = self.vars[-1] * (1 / batch_size) * torch.sum(errors[l + 1], axis=1)
            DW = errors[l + 1] @ F.f(x[l], self.act_fn).T
            # print("Layer l="+str(l)+" gives tensor update of shape "+str(DW.shape))
