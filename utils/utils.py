...
def loss_function_lu(args, net, use_jacobian_as_fb=False):
    """Compute the surrogate loss H on the last batch.

    The loss is computed as the norm of the amount of help received.

    Args:
        args: The config.
        net: The network.
        use_jacobian_as_fb (bool, optional): Whether the ideal feedback
            weights should be used to compute the loss.

    Returns:
        (float): The surrogate loss.
    """
    # J is (B, n_L, \sum_{i=1}^{L}n_{i})
    J = net.compute_full_jacobian(steady_state=True)
    u_ss = net.u
    ...
    for b in range(batchsize):
        ...
        loss_lu += torch.norm(torch.matmul(feedback_weights, u_ss[b,:].unsqueeze(1)))**2
    return loss_lu/batchsize
...
def save_logs(writer, step, net, loss, epoch_time, accuracy, test_loss, test_accuracy,
              val_loss, val_accuracy, loss_lu=None):
    """
    ...
    if val_loss is not None:
        writer.add_scalar(tag='training_metrics/val_loss',
                          ...
                          global_step=step)
    if loss_lu is not None:
        writer.add_scalar(tag='training_metrics/loss_lu',
                          scalar_value=loss_lu,
                          global_step=step)
...
def save_angles(args, writer, step, net, loss, output_activation):
    ...
    if args.save_BP_angle or args.save_correlations:
        retain_graph = args.save_GN_angle or args.save_GNT_angle or \
                       args.save_LU_angle or args.save_nullspace_norm_ratio or \
    ...
    if args.save_correlations:
        retain_graph = args.save_GN_angle or args.save_GNT_angle or \
                       ...
                       args.save_GN_GNT_angle or args.save_BP_GNT_angle or \
                       args.save_GNT_ss_no_ss_angle
        net.save_dfc_angles(writer, step,
                        save_dataframe=args.save_df,
                        save_tensorboard=args.save_logs)
        net.save_dfc_diff_hebbian_angles(writer, step,
                        save_dataframe=args.save_df,
                        save_tensorboard=args.save_logs)
    
    if args.save_GN_angle:
        retain_graph = args.save_GNT_angle or args.save_LU_angle or \
                       args.save_ratio_angle_ff_fb or \
                       args.save_fb_values or \
    ...
    if args.save_GNT_angle:
        retain_graph = args.save_nullspace_norm_ratio or args.save_LU_angle or \
                       args.save_ratio_angle_ff_fb or \
                       args.save_fb_values or \
                       args.save_GN_GNT_angle or args.save_BP_GNT_angle or \
                       args.save_GNT_ss_no_ss_angle
    ...
    if args.save_LU_angle:
        retain_graph = args.save_nullspace_norm_ratio or args.save_LU_angle or \
                       args.save_ratio_angle_ff_fb or \
                       args.save_fb_values or \
                       args.save_GN_GNT_angle or args.save_BP_GNT_angle or \
                       args.save_GNT_ss_no_ss_angle
        net.save_lu_angles(writer, step, loss, net.u,
                           retain_graph=retain_graph,
                           save_tensorboard=args.save_logs,
                           save_dataframe=args.save_df,
                           use_jacobian_as_fb=args.use_jacobian_as_fb)
    if args.save_ratio_angle_ff_fb:
        retain_graph = args.save_nullspace_norm_ratio or args.save_LU_angle or \
                       args.save_ratio_angle_ff_fb or \
                       args.save_fb_values or \
                       args.save_GN_GNT_angle or args.save_BP_GNT_angle or \
                       args.save_GNT_ss_no_ss_angle
        net.save_ratio_angle_ff_fb(writer, step, loss, net.u,
                             retain_graph=retain_graph,
                             save_tensorboard=args.save_logs,
                             save_dataframe=args.save_df)
    if args.save_fb_values:
        retain_graph = args.save_nullspace_norm_ratio or args.save_LU_angle or \
                       args.save_ratio_angle_ff_fb or \
                       args.save_fb_values or \
                       args.save_GN_GNT_angle or args.save_BP_GNT_angle or \
                       args.save_GNT_ss_no_ss_angle
        net.save_fb_values(writer, step, loss, net.u,
                             retain_graph=retain_graph,
                             save_tensorboard=args.save_logs,
                             save_dataframe=args.save_df)
    
    if args.save_NDI_angle and 'DFC' in args.network_type:
...
def save_batch_logs(args, writer, step, net, init=False,
                    statistics=False, weights='both'):
    """
    Save logs and plots for the current mini-batch on tensorboardX.
    ...
    Args:
        ...
        weights (str): The type of weights to be logged. Either `both`,
            `forward` or `feedback`.
    """
    assert weights in ['both', 'feedback', 'forward']

    if weights in ['both', 'feedback'] and not args.use_jacobian_as_fb:
        net.save_feedback_batch_logs(args, writer, step, init=init,
                                     save_tensorboard=args.save_logs,
                                     save_dataframe=args.save_df,
                                     save_statistics=True,
                                     damping=args.gn_damping)
    if weights in ['both', 'forward']:
        net.save_forward_batch_logs(args, writer, step,
                                     save_tensorboard=args.save_logs,
                                     save_statistics=True)
...
def save_summary_dict(args, summary, epoch=None):
    """
    ...
    with open(os.path.join(out_dir, summary_fn + '.txt'), 'w') as f:
        for k, v in summary.items():
            if k in ['pre_activities_vector', 'delta_post_activities_vector', 'weights_vector', 'weights_updates_vector', 'central_activities_vector']:
                pass
    ...
    with open(os.path.join(out_dir, summary_fn + '.pickle'), 'wb') as handle:
...
def process_hidden_activation(string):
    if ',' in string:
        ...
        for elem in lst:
            assert elem in ['tanh', 'relu', 'cap_relu', 'linear', 'leakyrelu', 'sigmoid']
        return lst
    else:
        assert string in ['tanh', 'relu', 'cap_relu', 'linear', 'leakyrelu', 'sigmoid']
        return string
...
class FastMNIST(MNIST):
    ...
    def __init__(self, root, device, double_precision, train=True,
